...
관여하는 엔드포인트는 둘뿐이고 둘 다 필드만 **additive** 로 는다.
...
배포 순간 `IN_PROGRESS` 로 남아 있던 토너먼트는 라운드 판정이 어긋난다. 조용히 깨지지는 않고 `invalidCurrentRound` 로 드러나지만 그 사용자는 진행하던 토너먼트를 끝내지 못한다. **트래픽이 적은 새벽에 배포해 노출을 줄인다.** 호환 코드는 두지 않는다(복잡도 대비 실익 없음).
