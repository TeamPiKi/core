...
# 소음 종류는 쿼리에 logger 제외를 추가해 뺀다. trace_id 를 묶음에 넣으면 건별 알림으로 되돌아간다.
resource "grafana_rule_group" "logs" {
  ...
  rule {
    ...
    data {
      ...
      model = <<-EOT
      {
        "expr": "sum by (service, environment, level, logger, error_type) (count_over_time({service=~\"piki-.+\", level=~\"(?i)(error|warn(ing)?|critical)\"} [10m]))",
        "intervalMs": 1000,
        "maxDataPoints": 43200,
        "queryType": "instant",
        "refId": "A"
...
# 라벨을 달지 않으니 루트 정책의 기본 경로인 discord-prod 로 나간다 (서버 로그 룰의 prod 와 같은 수신자).
