...
# 전 서비스 ERROR 건별 알림 - Sentry 대체 (#1002 후속). trace_id 로 묶어 에러마다 트레이스 링크를 싣는다.
resource "grafana_rule_group" "errors" {
  ...
  rule {
    name           = "서버 에러 로그 (전 서비스, level=ERROR)"
    condition      = "B"
    for            = "0s"
    no_data_state  = "OK"
    ...
    annotations = {
      ...
      summary   = "{{ $labels.service }} 에러{{ if $labels.error_type }} - {{ $labels.error_type }}{{ end }}{{ if $labels.logger }} ({{ $labels.logger }}){{ end }}"
    ...
    data {
      ...
      model = <<-EOT
      {
        "expr": "sum by (service, environment, level, error_type, logger, trace_id) (count_over_time({service=~\"piki-.+\", level=~\"(?i)(error|critical)\"} [10m]))",
        "intervalMs": 1000,
        "maxDataPoints": 43200,
        "queryType": "instant",
        "refId": "A"
...
locals {
  # 기동마다 고정으로 찍히는 무해한 WARN. 근본 해소는 #1111, 해소된 logger 는 여기서 지운다.
...
# 등록한 패턴만 보는 다른 룰과 달리 전 서비스 WARN 을 전부 잡는다 (#1105). 양이 많아 종류 단위로 묶는다.
resource "grafana_rule_group" "warnings" {
  name               = "warnings"
  folder_uid         = grafana_folder.piki_alerts.uid
  interval_seconds   = 60
  disable_provenance = true
  ...
  rule {
    name           = "서버 경고 로그 (전 서비스, level=WARN)"
    condition      = "B"
    for            = "0s"
    no_data_state  = "OK"
    exec_err_state = "Error"
    ...
    annotations = {
      logs_url = "https://piki.grafana.net/explore?schemaVersion=1&panes=%7B%22lg%22%3A%7B%22datasource%22%3A%22grafanacloud-logs%22%2C%22queries%22%3A%5B%7B%22refId%22%3A%22A%22%2C%22expr%22%3A%22%7Bservice%3D%5C%22{{ $labels.service }}%5C%22%2C%20environment%3D%5C%22{{ $labels.environment }}%5C%22%2C%20level%3D%5C%22{{ $labels.level }}%5C%22%7D%20%7C%20error_type%3D%5C%22{{ $labels.error_type }}%5C%22%20%7C%20logger%3D%5C%22{{ $labels.logger }}%5C%22%22%7D%5D%2C%22range%22%3A%7B%22from%22%3A%22__FROM__%22%2C%22to%22%3A%22__TO__%22%7D%7D%7D"
      summary  = "{{ $labels.service }} 경고{{ if $labels.error_type }} - {{ $labels.error_type }}{{ end }}{{ if $labels.logger }} ({{ $labels.logger }}){{ end }}"
    }
    data {
      ref_id         = "A"
      query_type     = "instant"
      ...
      model = <<-EOT
      {
        "expr": "sum by (service, environment, level, logger, error_type) (count_over_time({service=~\"piki-.+\", level=~\"(?i)warn(ing)?\"} | logger!~\"${join("|", local.startup_noise_loggers)}\" [10m]))",
        "intervalMs": 1000,
        "maxDataPoints": 43200,
        "queryType": "instant",
        "refId": "A"
...
# 라벨을 달지 않으니 루트 정책의 기본 경로인 discord-prod 로 나간다 (서버 에러 로그 룰의 prod 와 같은 수신자).
