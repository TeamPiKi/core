# Grafana 알림 구성 (Terraform)

**Grafana Cloud(piki.grafana.net) 의 알림 구성 정본은 이 디렉터리다** (이슈 #1008).
알림 룰 그룹 4개 · contact point 3개 · 루트 알림 정책 · 알림 폴더를 관리한다.
UI 에서 고칠 수는 있지만(잠금 없음) 다음 `apply` 가 코드 형상으로 되돌린다 —
UI 수정은 실험, 확정은 PR.

대시보드는 여기서 관리하지 않는다 (정본: `infra/grafana/*.json`, 반영 절차는 그 README).

## 기존 `terraform/` 과의 관계

| | `terraform/` (AWS) | `terraform-grafana/` (여기) |
|---|---|---|
| 대상 | EC2·VPC·S3·IAM 등 AWS 인프라 | Grafana Cloud 알림 구성 |
...
- **알림 변경은 PR -> 리뷰 -> apply.** apply 는 수동(로컬)이다.
