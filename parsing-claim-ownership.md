...
- **"원자적으로 일치시켜야 할 사실이 둘인가?"** 둘이면 outbox, 하나면 상태머신이다. 우리는 하나다 - outbox 가 푸는 문제(dual-write)가 발생하지 않는 구조이므로, 종 수준에서는 outbox 가 아니다.
- 코드 주석의 "transactional outbox" 는 2.3 의 보장 수준 명명으로 읽는다. 종 판정에 끌어다 쓰면 2.6 의 착시가 생긴다.
