...
- **푸는 문제**: dual-write. 도메인 DB 커밋과 외부 브로커 publish 는 서로 다른 시스템이라 원자적으로 묶을 수 없고, 사이에서 죽으면 "커밋은 됐는데 발행은 안 된" 미아가 생긴다.
- **해법**: 발행할 메시지를 도메인 write 와 **같은 로컬 트랜잭션**으로 outbox 테이블에 적어 두고, relay 가 그 행을 읽어 브로커에 publish 한다.
...
- **푸는 문제**: 선언된 상태와 현실의 수렴. "PENDING = 아직 파싱 안 됨"이라는 상태 술어 자체가 작업 정의이고, 폴러는 그 차이를 계속 줄이는 조정 루프(reconciliation loop)다. k8s 컨트롤러와 같은 족보.
...
- 진화 경로 예측도 갈린다(2.4 마지막 행): 우리가 부하로 인해 이 구조를 벗어나는 날의 착지점은 발행-구독이 아니라 잡 큐(4.2 의 SQS 대응표)다.
