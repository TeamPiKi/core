...
// 시계는 sleep 이 아니라 parse_requests.heartbeat_at 데이터 조작으로 돌린다(stale 판정은 threshold 를 직접 넘겨 재현).
class ParsingHeartbeatTimingIntegrationTest : IntegrationTestSupport() {
    ...
    @Test
    fun `박동(renew)은 소유권을 쥔 PROCESSING 의 heartbeat_at 을 밀어 stale 판정에서 빼낸다`() {
        val item = itemRepository.save(Item(ProductLink.parse("https://shop.example.com/products/touch-${UUID.randomUUID()}")))
        val requestId = enqueueProcessing(item.getId()) // attempt 0 (집기는 예산 미소모)
        try {
            assertEquals(1, parsingOwnership.acquire(requestId, 0), "실행 진입 시 소유권을 획득해야 한다")
            // threshold 를 지금으로 잡으면 그 직전에 갱신된 행(heartbeat_at < now)은 stale 로 잡힌다.
            val threshold = LocalDateTime.now()
            assertTrue(requestId in staleIds(threshold), "renew 전에는 threshold 이전이라 stale 로 잡혀야 한다")

            // 박동(renew) — 소유권(attempt 1) 유지라 1행 매치, heartbeat_at 이 threshold 이후로 밀린다.
            assertEquals(1, parsingOwnership.renew(requestId, 1), "소유권을 쥔 PROCESSING 은 1행 매치여야 한다")

            assertFalse(requestId in staleIds(threshold), "renew 후에는 heartbeat_at 이 threshold 를 넘어 더는 stale 이 아니다")
        } finally {
    ...
    fun `beat 는 소유권을 쥔 산 항목을 갱신해 stale 에서 빼고 레지스트리에 유지한다`() {
        ...
        val requestId = enqueueProcessing(item.getId()) // attempt 0
        try {
            assertEquals(1, parsingOwnership.acquire(requestId, 0), "실행 진입 시 소유권을 획득해야 한다")
            // 등록 전(auto-beat 대상 아님)에 threshold 를 잡으면 그 직전 갱신된 행은 그 시점 이전이라 stale 로 잡힌다 — 결정론.
            val threshold = LocalDateTime.now()
            assertTrue(requestId in staleIds(threshold), "beat 전(생성 직후)에는 threshold 이전이라 stale 이어야 한다")

            parsingHeartbeat.register(requestId, 1)
            parsingHeartbeat.beat()

            assertFalse(requestId in staleIds(threshold), "beat 의 정상 분기가 touch 해 heartbeat_at 을 threshold 뒤로 밀어야 한다")
            assertTrue(parsingHeartbeat.isTracking(requestId), "소유권을 쥔 산 항목은 beat 후에도 레지스트리에 남아야 한다")
        } finally {
            parsingHeartbeat.deregister(requestId, 1)
            deleteItem(item.getId())
        }
    }
    ...
    fun `소유권이 넘어간 행에 대한 옛 시도의 박동은 0행이고 beat 가 그 좀비를 레지스트리에서 제거한다`() {
        ...
        val requestId = enqueueProcessing(item.getId()) // attempt 0
        try {
            // 소유권이 다른 시도로 넘어간 상황 재현 — 새 시도가 attempt 2 로 올리고 박동까지 민 상태다.
            jdbcTemplate.update(
            ...
            assertEquals(0, parsingOwnership.renew(requestId, 1), "소유권이 넘어간 행에 옛 attempt 로 박동하면 0행이어야 한다")

            // 그 좀비가 레지스트리에 남아 박동하면 beat 가 0행을 보고 제거한다.
            parsingHeartbeat.register(requestId, 1)
            parsingHeartbeat.beat()
            assertFalse(parsingHeartbeat.isTracking(requestId), "beat 가 소유권 잃은 좀비를 레지스트리에서 제거해야 한다")
        } finally {
            parsingHeartbeat.deregister(requestId, 1)
            deleteItem(item.getId())
        }
    ...
    private fun staleIds(threshold: LocalDateTime): List<Long> =
        TransactionTemplate(transactionManager).execute {
            parseRequestRepository.findStaleProcessing(threshold, 200).map { it.getId() }
        } ?: emptyList()

    // 요청 + 결과 버전 한 벌을 집힌 직후(PROCESSING, attempt 0)로 적재하고 요청 id 를 돌려준다. 박동·stale 판정 대상은 요청 행이다.
    private fun enqueueProcessing(itemId: Long): Long {
        val snapshot =
            itemSnapshotRepository.save(
                ItemSnapshot.pending(itemId, requestedBy = UUID.randomUUID()).apply { markProcessing() },
            )
        return parseRequestRepository
            .save(ParseRequest(itemId, UUID.randomUUID(), ParseTrigger.REGISTER, snapshot.getId()).apply { claim() })
            .getId()
    }
