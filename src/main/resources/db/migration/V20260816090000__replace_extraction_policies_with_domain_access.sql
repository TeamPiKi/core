...
--
-- 왜 이관하지 않고 새로 만드나: 옛 route 3값과 허가 boolean 은
-- 서로 다른 질문(등록을 받나 / 어떻게 가져오나 / 확인했나 / 어디까지 허락됐나)에 한 축으로 답하고 있어
...
--
-- ALLOWED 는 "플랫폼의 명시적 허락을 받았다"는 뜻이고, 그 결과로 추출 모듈이 적극적인 수단까지 쓸 수 있게
-- 된다. 허락은 사람이 받아 오는 것이라 근거(permission_ref) 없이는 켤 수 없다 — 입력 경계가 그걸 강제한다.
