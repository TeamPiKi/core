...
data class ItemQuotaProperties(
    ...
    // (실측이 아니라 timeout 상한에서 잡은 추정). 무거운 경로(LLM 등)가 섞이면 건당 5~20초까지 늘어 이론 처리량이
    // 시간당 1,400건까지 떨어지는데, 그 구간에서는 이 상한이 워커보다 느슨해 상한에 닿기 전에 PENDING 이 쌓인다.
    // 거부가 아니라 대기라 장애는 아니지만, 대부분의 등록이 파서로 풀리는 것을 전제로 잡은 값이다.
