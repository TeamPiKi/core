...
// docs.enabled = ${SPRINGDOC_ENABLED:true} (deploy.yml 이 dev 만 true) — dev 만 문서를 노출하고,
// staging/prod 는 이 config 빈 자체가 안 떠 resource handler·리다이렉트가 없으므로 /docs·/ 가 404 다.
// spec(/v3/api-docs)은 springdoc.api-docs.enabled(같은 플래그)로 별도 게이팅된다. 로컬·test 는 기본 true.
//
...
@Configuration
@ConditionalOnProperty(name = ["docs.enabled"], havingValue = "true", matchIfMissing = true)
