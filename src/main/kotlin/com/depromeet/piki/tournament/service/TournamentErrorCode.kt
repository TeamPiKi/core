...
enum class TournamentErrorCode(
    ...
    // 038 은 #977. 복제 토너먼트는 자기 아이템 행이 없고 원본 것을 이어받아 보여준다. 단건 조회를 원본 기준으로
    // 뚫어 주면서(같은 이슈) 쓰기까지 함께 뚫리면 남의 원본 아이템을 고치게 되므로, 쓰기 경로는 명시적으로 막는다.
    // 추가 금지(032)와 같은 결이고, 종전의 "소속 불일치 404" 를 대체한다 — 404 는 아이템이 없다는 뜻이라 오해를 낳았다.
    // 수정·삭제를 한 code 로 묶는다: 사용자에게는 "복제본에선 원본 아이템을 건드릴 수 없다" 는 한 가지 사실이다.
