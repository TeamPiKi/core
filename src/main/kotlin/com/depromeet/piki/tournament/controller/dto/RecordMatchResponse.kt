...
//
// NON_NULL 이라 null 필드는 JSON 에서 `"nextMatch": null` 이 아니라 **키째 생략**된다 (레포 공통 규약).
// 두 필드가 모두 비는 라운드 종료 응답은 `"data": {}` 다. 문서·example·클라 타입을 이 생략 계약에 맞춘다.
@JsonInclude(JsonInclude.Include.NON_NULL)
data class RecordMatchResponse(
    // 같은 라운드에 남은 다음 매치. 생략되면 라운드가 끝났다는 뜻이고,
    // 클라이언트는 GET /tournaments/{id} 를 다시 불러 다음 라운드를 받는다.
    val nextMatch: TournamentDetailResponse.MatchResponse?,
    // 결승 매치를 기록한 경우에만 채워지는 본인 순위 결과. 그 외에는 생략된다.
