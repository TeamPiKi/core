...
// .resolveRefreshContexts), 등록 완료 알림은 그들을 뺀다(resolveRegistered). 갈리는 근거는 위시 생성시각이 버전보다
...
class ItemRefreshCompletedHandler(
    ...
    // 라우팅은 수신자별 자기 위시(wishId). dispatcher 가 수신자 도출과 이 해석을 따로 부르므로 같은 조회가 이벤트당
    // 두 번 돈다(다른 파싱 핸들러와 같은 비용 구조) — 수신자 수에는 비례하지 않는다.
