...
interface NotificationSseApi {
    @Operation(
        ...
        description =
            "인증 유저의 알림을 실시간으로 받는 **SSE(Server-Sent Events)** 스트림을 연다.\n\n" +
                ...
                "- `notification`·`silent-sync` 이벤트에는 **SSE `id` 필드**(불투명 문자열)가 실린다. " +
                "**재연결 시 `Last-Event-ID` 헤더**(마지막으로 받은 이벤트의 `id`)를 보내면 끊김 동안 쌓인 이벤트를 " +
                "종류 구분 없이 발생 순서대로 다시 흘려보낸다(최대 100건 — 초과 공백은 replay 없이 목록/배지 API 재조회로 복구). " +
                "라이브 전송과 겹쳐 같은 이벤트가 중복 도착할 수 있으므로 클라이언트는 이벤트 `id` 로 dedup 한다. " +
                "상세 계약은 `notification-sse-spec.md` 참조.",
    )
    @ApiResponses(
        value = [
            ApiResponse(
    ...
    fun subscribe(
        ...
        @Parameter(
            ...
            required = false,
            description =
                "재연결 시 마지막으로 받은 이벤트의 SSE `id`(불투명 문자열). " +
                    "브라우저 `EventSource` 는 자동으로 싣고, APP 직접 구현은 마지막 id 를 저장했다가 싣는다. " +
                    "없거나 형식이 다르면 첫 연결로 취급한다(replay 없음).",
