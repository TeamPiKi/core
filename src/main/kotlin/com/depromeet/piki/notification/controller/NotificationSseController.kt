...
class NotificationSseController(
    ...
    override fun subscribe(
        ...
        // register 를 replay 조회보다 먼저 둔다 — 반대 순서(조회 후 register)면 그 틈에 발행된 라이브 이벤트가
        // 유실된다. 이 순서로는 같은 이벤트가 라이브·replay 양쪽으로 겹칠 수 있으나(중복), 클라이언트가
        // 이벤트 id 로 dedup 하는 계약이다(notification-sse-spec.md). 유실보다 중복이 옳다.
        ...
        // 재연결이면(Last-Event-ID) 끊김 동안 놓친 이벤트를 이 연결에만 replay 한다.
        // 헤더 파싱은 보수적으로 — SSE 프로토콜상 id 재개는 서버가 지원하지 않을 수도 있는 optional 계약이라,
        // 형식(stream entry id)이 아닌 값은 400 으로 끊지 않고 "첫 연결" 로 취급한다(재연결 루프를 깨지 않는다).
        lastEventIdHeader?.takeIf(EVENT_ID_PATTERN::matches)?.let { replayer.replayMissed(userId, emitter, it) }
        return emitter
    }
    ...
    companion object {
        ...
        // SSE 표준 재연결 헤더 — EventSource 가 마지막 수신 이벤트 id 를 자동으로 싣는다.
        const val HEADER_LAST_EVENT_ID = "Last-Event-ID"

        // 이벤트 id(Redis stream entry id, 예: "1752741234567-0") 형식. 이 형식이 아니면 첫 연결로 취급한다.
        private val EVENT_ID_PATTERN = Regex("""\d+-\d+""")
    }
}
