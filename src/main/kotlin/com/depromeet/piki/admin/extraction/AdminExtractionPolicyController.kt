...
class AdminExtractionPolicyController(
    ...
    @GetMapping
    fun board(
        @RequestParam(required = false) access: String?,
    ...
    fun add(
        ...
        @RequestParam access: DomainAccess,
        @RequestParam(required = false) reason: String?,
        request: HttpServletRequest,
        model: Model,
    ): String =
        try {
            adminExtractionPolicyService.save(domain, access, reason, actor = AdminSession.actorName(request), clientIp = ClientIp.of(request))
            "redirect:/admin/extraction-policies?updated"
        } catch (e: IllegalArgumentException) {
            // 정규화·길이 검증 실패 — 제출값(access 선택 포함)을 유지한 채 보드 화면에 에러를 표시한다(400 JSON 대신 SSR).
            model.addAttribute("error", e.message)
            model.addAttribute("draftDomain", domain)
            model.addAttribute("draftReason", reason)
            boardView(filter = null, model = model, selectedAccess = access.name)
        }
    ...
    private fun boardView(
        ...
        model.addAttribute("board", adminExtractionPolicyService.board(parseAccess(filter)))
        model.addAttribute("accesses", DomainAccess.entries)
        model.addAttribute("selectedAccess", selectedAccess ?: DomainAccess.BLOCKED.name)
    ...
    private fun detailView(
        ...
        model.addAttribute("accesses", DomainAccess.entries)
        model.addAttribute("selectedAccess", selectedAccess ?: policy.access)
        model.addAttribute("knownAccess", DomainAccess.entries.any { it.name == policy.access })
        model.addAttribute("permissionRefValue", draftPermissionRef ?: policy.permissionRef)
        return "admin/extraction-policy-detail"
    }

    // ?access= 는 tolerant 하게 읽는다 — 모르는 값이면 400 대신 "필터 없음(전체)"으로 떨군다. 옛 링크를 눌렀거나
    // URL 을 손으로 고쳤을 때 화면이 깨지는 것보다, 전체 보드를 보여주는 편이 백오피스에서 덜 위험하다.
    private fun parseAccess(raw: String?): DomainAccess? = DomainAccess.entries.find { it.name == raw }
}
