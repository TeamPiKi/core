...
import com.depromeet.piki.product.service.remote.RemoteExtractionResponse
...
// 계약은 링크와 같은 3갈래(RemoteExtractionContract)이고 요청만 다르다 — URL 대신 raw 가 적재된 S3 위치(bucket·key)를
// 넘기면 extractor 가 download→OCR→crop→업로드까지 끝내고 업로드된 결과 imageUrl 을 돌려준다.
...
class HttpImageSnapshotExtractor(
    ...
    override fun extract(imageKey: String): ProductSnapshot {
        val target = "key=$imageKey"
        // 전환기 롤아웃 관측용 — 어떤 이미지가 원격을 탔는지의 원장(링크의 route=remote 로그와 같은 결).
        // key 는 내부 식별자(items/raw/{uuid})라 로그에 안전하다.
        log.info("image extract route=remote {}", target)
        val response =
            try {
                restClient
                    .post()
                    .uri(IMAGE_EXTRACTION_PATH)
                    .contentType(MediaType.APPLICATION_JSON)
                    .body(RemoteImageExtractionRequest(s3Properties.bucket, imageKey))
                    .retrieve()
                    .body(RemoteExtractionResponse::class.java)
            } catch (e: RestClientResponseException) {
                throw RemoteExtractionContract.translate(e, target)
            } catch (e: RestClientException) {
                // 연결 실패·read timeout 등 transport 장애 — 일시로 본다(워커가 PROCESSING 유지 → recover 재시도).
        ...
        return RemoteExtractionContract.toSnapshot(response, link = null, target = target)
    }
