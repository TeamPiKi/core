...
interface WishlistApi {
    ...
    @Operation(
        ...
        description = """
            ...
            priceHistory 는 최신순으로 최대 50건이며, **서버가 추출해 관측한 가격(SERVER·SERVER_LLM)과 본인이 직접 입력한 값(MANUAL)** 을 담는다.
            같은 상품을 담은 **타인의 수기와 출처 기록 도입 전 버전은 빠진다** — 전자는 남의 위시 사정이고, 후자는 관측인지 입력인지 소급 판정할 수 없기 때문이다.
            그래서 응답에 편집자 식별자를 싣지 않는다. 내려온 MANUAL 은 곧 본인이 넣은 값이다.

            **본인이 입력한 값도 이력의 1급 구성원이다** — 접거나 걸러낼 대상이 아니다. 서버 추출값도 완전하지 않고(특히 SERVER_LLM 은 같은 페이지를
            같은 날 다시 긁어도 값이 달라질 수 있다), 사용자는 페이지를 직접 보고 자기에게 유효한 가격(쿠폰·회원가·옵션 반영)을 적기 때문이다.
            추출이 계속 실패하는 상품에서는 그것이 유일한 기록이기도 하다.
            source 는 신뢰도 등급이 아니라 **맥락 표시**다. 둘이 서로 다른 값을 재고 있을 수 있으므로(페이지 표시가 vs 사용자의 실구매가)
            마커 모양·색 등으로 구분해 보여주기를 권한다.

            표시 버전이 READY 인 한 priceHistory 의 첫 항목이 곧 지금 보이는 값이다. 추출 진행 중(PENDING·PROCESSING)이거나 실패(FAILED)면 그 값은 목록에 없고,
            한 번도 추출에 성공하지 못했고 직접 입력한 적도 없으면 빈 배열이다.
        """,
    )
    @ApiResponses(
        value = [
