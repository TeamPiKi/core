...
class WishPersistenceService(
    ...
    // 등록 전 사전 확인 — 이미 담은 상품이면 한도를 깎기 전에 거를 수 있도록 그 위시의 id 를 돌려준다(#973).
    // 없으면 null(신규 등록으로 진행).
    //
    ...
    fun findExistingWishId(
        userId: UUID,
        link: ProductLink,
    ): Long? {
        val shared = itemSharingService.resolveExistingItem(link) ?: return null
        return wishRepository
            .findByItemIdsAndUserId(listOf(shared.getId()), userId)
            .firstOrNull()
            ?.getId()
    }
    ...
    fun persist(
        ...
        item.link?.let { link ->
            itemSharingService.resolveExistingItem(link)?.let { shared ->
                ...
                // 이미 담긴 위시의 id 를 응답에 실어야 하므로 존재 여부(count)가 아니라 그 행을 찾는다(#973).
                // itemId 는 유저 내에서 사실상 1:1 이라 첫 건이 곧 그 위시다.
                wishRepository.findByItemIdsAndUserId(listOf(attachment.item.getId()), userId).firstOrNull()?.let {
                    throw WishException.alreadyExists(it.getId())
                }
