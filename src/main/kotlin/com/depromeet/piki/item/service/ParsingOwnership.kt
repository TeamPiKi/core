...
// 파싱 단건의 **소유권** 을 다루는 짧은 트랜잭션 경계 — 획득(acquire)과 갱신(renew) 두 가지다. 대상은 요청 행이다(#1073 2단계).
//
...
//     집혔지만 제출이 거부돼 실행이 0회인 요청은 예산을 잃지 않는다.
//   - renew  : 실행 중 살아있음을 새긴다(박동). attempt 는 건드리지 않아 소유권이 옮겨가지 않는다.
// 소유권 반납은 별도 연산이 없다 — 전이(성공·실패)가 status 를 터미널로 바꿔 두 조건을 동시에 깨뜨린다.
//
...
class ParsingOwnership(
    ...
    fun acquire(
        ...
        val acquired = parseRequestRepository.acquireOwnership(requestId, expectedAttempt, LocalDateTime.now())
    ...
    fun renew(
        requestId: Long,
        attempt: Int,
    ): Int = parseRequestRepository.renewOwnership(requestId, attempt, LocalDateTime.now())
}
