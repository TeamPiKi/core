...
//
// 레지스트리 키는 requestId 다. 되살림으로 같은 요청의 옛 시도와 새 시도가 잠깐 공존할 수 있어(죽은 줄 알았던
...
class ParsingHeartbeat(
    ...
    // requestId → 이 실행이 쥔 fencing 토큰(attempt). 박동은 이 토큰이 여전히 유효할 때만 먹힌다.
    ...
    fun register(
        ...
        registry.merge(requestId, attempt) { old, new -> maxOf(old, new) }
    }
    ...
    fun deregister(
        ...
        registry.remove(requestId, attempt)
    }
    ...
    fun guarded(
        ...
        val attempt = parsingOwnership.acquire(requestId, expectedAttempt) ?: return onOwnershipLost()
        register(requestId, attempt)
        try {
            body(attempt)
        } finally {
            deregister(requestId, attempt)
        }
    }
    ...
    fun beat() {
        registry.forEach { (requestId, attempt) ->
            runCatching { beatOne(requestId, attempt) }
                .onFailure { e -> log.warn("request {} 박동 실패 — 다음 사이클 재시도 (attempt={}): {}", requestId, attempt, e.message) }
        }
    }
    ...
    private fun beatOne(
        ...
        if (parsingOwnership.renew(requestId, attempt) == 0) {
            ...
            registry.remove(requestId, attempt)
            log.info("request {} 박동 대상 아님(되살림·이미 전이) — 레지스트리 제거 (attempt={})", requestId, attempt)
        }
    }

    // 테스트가 레지스트리 상태를 관측하는 최소 표면.
    internal fun isTracking(requestId: Long): Boolean = registry.containsKey(requestId)
